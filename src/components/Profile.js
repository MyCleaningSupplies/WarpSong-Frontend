...
// In Profile.js, modify the useEffect
useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // ✅ Ensure stems is always an array
        if (!data.stems) {
          data.stems = [];
        }
    ...
    <div className="min-h-screen flex flex-col items-center bg-gradient-to-b from-purple-700 to-pink-500 p-6 text-white">
      <div className="w-full max-w-2xl bg-white/10 backdrop-blur-lg p-6 rounded-2xl shadow-xl">
        ...
        <div className="flex items-center justify-between mb-6">
          <button onClick={() => navigate("/")} className="text-white text-lg">← Terug</button>
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="text-sm text-gray-300">Lid sinds: {profile.createdAt ? new Date(profile.createdAt).toLocaleDateString() : "Onbekend"}</p>
        ...
        <div className="mt-6">
          ...
          <p className="text-sm mt-2">{profile.xp || 0} / {(profile.level || 1) * 50} XP</p>
        </div>
            ...
            {profile.stems.length > 0 ? (
              profile.stems.map((stem, index) => (
                <div key={index} className="p-3 bg-white/10 rounded-xl">
                  <p className="text-sm">{stem.name} - {stem.type}</p>
                </div>
              ))
            ) : (
